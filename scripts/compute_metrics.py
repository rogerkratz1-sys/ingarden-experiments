...
def disp_kappa_scaled(perm, canon, kappa, alpha=0.0001):
    disp = disp_kappa(perm, canon, kappa)
...
def main():
    ...
    with open(args.out, "w", newline="", encoding="utf-8") as outf:
        writer = csv.DictWriter(outf, fieldnames=list(rows[0].keys()) if rows else [])
        if rows:
            writer.writeheader()
            for r in rows:
                writer.writerow(r)
